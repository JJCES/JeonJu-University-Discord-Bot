...
dotenv.config();
import { ActionRowBuilder, ActivityType, ButtonBuilder, ButtonStyle, ChannelType, Client, Colors, EmbedBuilder, GatewayIntentBits, GuildMember, ModalBuilder, PermissionFlagsBits, TextInputBuilder } from 'discord.js';
    ...
    if (interaction.isButton()) {
        ...
        switch (componentCommand) {
            ...
            case "instarLogin":
                const embed = new EmbedBuilder({
                    title: "가입전 꼭 확인해주세요!",
                    description: "<#1014568783345623050>을 꼭 읽어주세요!\n사용자에 대한 규칙만이 아닌 개인정보 수집 범위 등을 표기하고 있습니다.\n규칙을 다 읽으셨다면 아래 버튼을 통해 로그인 해주세요.\n\n\`로그인시 위 규칙에 명시된 개인정보 수집과 규칙에 대해 동의한 것으로 간주됩니다.\`",
                    color: Colors.Red
                });
        ...
        switch (componentCommand) {
            case "support":
                ...
                const channel = interaction.guild?.channels.cache.get("1065544996545105931");
            ...
            case "verifyInstar":
                ...
                if (!id || !pw) {
                    ...
                    // 학과 정보 등록 
                    const majorRole = guildRoles.cache.find(i => i.name == info[1].trim());
                    if (majorRole) {
                    ...
                    // 스타 역할 적용
                    const starRole = guildRoles.cache.find(i => i.id == "1014571013788409999");
                    member.roles.add(starRole!);
        ...
        switch (command) {
            ...
            case "건의":
                ...
                break;
            case "로그인_폼_등록":
                const channel = interaction.guild?.channels.cache.get(env.WELCOME_CHANNEL_ID);

                if (channel?.type === ChannelType.GuildText) {
                    const embed = new EmbedBuilder({
                        title: "전주대학생 인증",
                        description: "무분별한 가입을 막기 위하여 인스타 인증기능을 지원하고 있습니다.\n아래 버튼을 눌러 인스타 아이디와 비밀번호를 입력하여 전주대 학생임을 인증해주세요.\n\n\`[아이디와 비밀번호는 인증 이후 기록에서 삭제되오니 걱정하지 마시길 바랍니다.]\n로그인시 학년, 학과 정보가 역할에 적용되며 자동으로 닉네임이 본명으로 변경됩니다.\`",
                        color: Colors.Gold
                    });

                    const component = new ActionRowBuilder<ButtonBuilder>().addComponents(
                        new ButtonBuilder().setCustomId("instarLogin").setLabel("인스타 인증").setStyle(ButtonStyle.Primary)
                    );
...
connect("mongodb://127.0.0.1:27017/Jj").then(() => {
